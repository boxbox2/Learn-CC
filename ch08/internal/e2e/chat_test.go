...
func TestToolReadRoundTrip(t *testing.T) {
	...
	events, err := session.Submit(context.Background(), "read note", chat.SubmitModeDefault)
	if err != nil {
		t.Fatal(err)
	}
	var content string
	...
	if len(fp.requests[1].Tools) == 0 {
		t.Fatalf("second request should keep tools available for the agent loop")
	}
	if len(session.History) != 4 || len(session.History[2].ToolResults) != 1 {
...
func TestFakeStreamingConversationAndRetry(t *testing.T) {
	...
	events, err := session.Submit(context.Background(), "hello", chat.SubmitModeDefault)
	if err != nil {
		t.Fatal(err)
	}
	...
	retryEvents, err := session.Retry(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	for range retryEvents {
	}
	if len(fp.requests) != 2 {
		t.Fatalf("requests = %d, want 2", len(fp.requests))
	}
