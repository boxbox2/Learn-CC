...
func (App) RunChat(ctx context.Context, opts ChatOptions) error {
	...
	skillCatalog, err := skill.Load(skill.LoadOptions{
	...
	if err := registry.Register(skill.LoadSkillTool{Catalog: skillCatalog, Active: activeSkills}); err != nil {
		return fmt.Errorf("register LoadSkill: %w", err)
	}
	if err := skill.RegisterSkillCommands(commands, skillCatalog, nil); err != nil {
		return fmt.Errorf("register skill commands: %w", err)
	}
