...
func systemBlocks(req provider.ChatRequest) []anthropicsdk.TextBlockParam {
	var blocks []anthropicsdk.TextBlockParam
	if req.SystemPrompt != "" {
		blocks = append(blocks, anthropicsdk.TextBlockParam{Text: req.SystemPrompt})
	}
	for _, note := range req.SystemNotes {
		if note.Content != "" {
			blocks = append(blocks, anthropicsdk.TextBlockParam{Text: note.Content})
		}
	}
	return blocks
}
