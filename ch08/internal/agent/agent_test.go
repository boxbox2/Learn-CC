...
func (testPromptContext) CustomInstructions(ctx context.Context) string { return "custom instructions" }
func (testPromptContext) LongTermMemory(ctx context.Context) string     { return "long term memory" }
...
func (r *recordingRecorder) Append(msg provider.ChatMessage) error {
	if r.err != nil {
		return r.err
	}
...
func TestRunnerUsesPromptContext(t *testing.T) {
	fp := &fakeProvider{responses: [][]provider.StreamEvent{
		{{Type: provider.StreamEventTypeTextDelta, Delta: "done"}, {Type: provider.StreamEventTypeDone}},
	}}
	runner := &Runner{
	...
	if !strings.Contains(fp.requests[0].SystemPrompt, "custom instructions") || !strings.Contains(fp.requests[0].SystemPrompt, "long term memory") {
		t.Fatalf("system prompt = %q", fp.requests[0].SystemPrompt)
	}
}

func TestRunnerRecorderRecordsNewMessages(t *testing.T) {
	reg := tool.NewRegistry()
	if err := reg.Register(fakeTool{name: "Read", safety: tool.SafetyReadOnly}); err != nil {
		t.Fatal(err)
	}
	...
	runner := &Runner{Provider: fp, Config: config.ProviderConfig{Model: "fake"}, Tools: reg, Recorder: recorder}
	run, err := runner.Run(context.Background(), RunRequest{
		Messages: []provider.ChatMessage{{Role: provider.RoleUser, Content: "go"}},
		Tools:    ToolSet{Mode: ToolSetAll},
	})
	...
	if len(recorder.messages) != 3 {
		t.Fatalf("recorded messages = %+v", recorder.messages)
	}
	if len(recorder.messages[0].ToolCalls) != 1 || len(recorder.messages[1].ToolResults) != 1 || recorder.messages[2].Content != "done" {
		t.Fatalf("recorded messages = %+v", recorder.messages)
	}
}

func TestRunnerRecorderErrorStopsRun(t *testing.T) {
	fp := &fakeProvider{responses: [][]provider.StreamEvent{
		{{Type: provider.StreamEventTypeTextDelta, Delta: "done"}, {Type: provider.StreamEventTypeDone}},
	}}
	runner := &Runner{Provider: fp, Config: config.ProviderConfig{Model: "fake"}, Tools: tool.NewRegistry(), Recorder: &recordingRecorder{err: fmt.Errorf("record failed")}}
	run, err := runner.Run(context.Background(), RunRequest{
		Messages: []provider.ChatMessage{{Role: provider.RoleUser, Content: "go"}},
		Tools:    ToolSet{Mode: ToolSetAll},
	})
	...
	drainEvents(run.Events)
	result := <-run.Done
	if result.Stop != StopStreamError || result.Err == nil {
		t.Fatalf("result = %+v", result)
	}
}
...
func TestRunnerEmergencyCompactsAndRetriesOnce(t *testing.T) {
	fp := &fakeProvider{
		errs: []error{errors.New("prompt_too_long")},
		responses: [][]provider.StreamEvent{
			{
	...
	runner := &Runner{Provider: fp, Config: cfg, ContextManager: manager, Options: Options{MaxIterations: 1}}
	run, err := runner.Run(context.Background(), RunRequest{
		Messages: []provider.ChatMessage{{Role: provider.RoleUser, Content: "go"}},
		Tools:    ToolSet{Mode: ToolSetAll},
	})
	...
	if !sawContext {
		t.Fatal("expected emergency context event")
	}
	if len(fp.requests) != 3 {
		t.Fatalf("requests = %d, want original + summary + retry", len(fp.requests))
	}
	if len(fp.requests[1].Tools) != 0 {
		t.Fatalf("summary request tools = %+v, want none", fp.requests[1].Tools)
	}
}
