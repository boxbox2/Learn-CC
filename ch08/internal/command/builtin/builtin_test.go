...
func (f *fakeController) ListCatalogSkills() []command.SkillSummary {
	return append([]command.SkillSummary(nil), f.skills...)
}
func (f *fakeController) RunSkill(ctx context.Context, name, args string) error {
	f.runSkill = append(f.runSkill, name+" "+args)
	return nil
}
...
func TestRegisterBuiltins(t *testing.T) {
	...
	for _, name := range []string{"/help", "/compact", "/clear", "/plan", "/do", "/session", "/memory", "/permission", "/status", "/skill", "/exit"} {
...
func TestCompactClearExit(t *testing.T) {
	reg := command.NewRegistry()
	Register(reg)
	reg.MustValidate()
	fc := &fakeController{}
	for _, name := range []string{"/compact", "/clear", "/exit"} {
		def, _ := reg.Lookup(name)
		if _, err := def.Handler(context.Background(), command.Invocation{Definition: def}, fc); err != nil {
			t.Fatalf("%s: %v", name, err)
		}
...
func TestSkillCommandListsCatalog(t *testing.T) {
	reg := command.NewRegistry()
	Register(reg)
	reg.MustValidate()
	fc := &fakeController{skills: []command.SkillSummary{{Name: "review", Description: "Review changes", Active: true}}}
	def, _ := reg.Lookup("/skill")
	if _, err := def.Handler(context.Background(), command.Invocation{Definition: def}, fc); err != nil {
		t.Fatal(err)
	}
	out := strings.Join(fc.messages, "\n")
	if !strings.Contains(out, "Available skills (1):") || !strings.Contains(out, "/review") || !strings.Contains(out, "[active]") {
		t.Fatalf("skill output = %q", out)
	}
}

func TestSkillCommandEmpty(t *testing.T) {
	reg := command.NewRegistry()
	Register(reg)
	reg.MustValidate()
	fc := &fakeController{}
	def, _ := reg.Lookup("/skill")
	if def.Kind != command.KindLocal {
		t.Fatalf("/skill kind = %s, want local", def.Kind)
	}
	if _, err := def.Handler(context.Background(), command.Invocation{Definition: def}, fc); err != nil {
		t.Fatal(err)
	}
	if strings.Join(fc.messages, "\n") != "No skills loaded." {
		t.Fatalf("messages = %+v", fc.messages)
	}
}
