...
func convertChunk(chunk openaisdk.ChatCompletionChunk) []provider.StreamEvent {
	...
	if chunk.Usage.PromptTokens != 0 || chunk.Usage.CompletionTokens != 0 || chunk.Usage.TotalTokens != 0 || chunk.Usage.PromptTokensDetails.CachedTokens != 0 {
...
func requestMessages(req provider.ChatRequest) []openaisdk.ChatCompletionMessageParamUnion {
	out := make([]openaisdk.ChatCompletionMessageParamUnion, 0, 1+len(req.SystemNotes)+len(req.Messages))
	if req.SystemPrompt != "" {
		out = append(out, openaisdk.SystemMessage(req.SystemPrompt))
	}
	for _, note := range req.SystemNotes {
		if note.Content != "" {
			out = append(out, openaisdk.SystemMessage(note.Content))
		}
	}
	out = append(out, toMessages(req.Messages)...)
	return out
}
