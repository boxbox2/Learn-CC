...
func AllowedDefinitions(registry *tool.Registry, set ToolSet) []tool.Definition {
	...
	case ToolSetReadOnly:
		return registry.VisibleDefinitionsBySafety(tool.SafetyReadOnly)
	default:
		return registry.VisibleDefinitions()
	}
}
...
func DeferredToolNames(registry *tool.Registry, set ToolSet) []string {
	...
	switch set.Mode {
	case ToolSetReadOnly:
		return registry.DeferredNamesBySafety(tool.SafetyReadOnly)
	default:
		return registry.DeferredNames()
	}
}
