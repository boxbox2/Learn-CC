...
func (s *Session) submitPlan(ctx context.Context, input string) (<-chan provider.StreamEvent, error) {
	content := "Create an execution plan for this task. You may inspect the project with read-only tools, but do not modify files or run commands.\n\nTask:\n" + input
	messages := append([]provider.ChatMessage(nil), s.History...)
	messages = append(messages, provider.ChatMessage{Role: provider.RoleUser, Content: content})
...
func (s *Session) submitDo(ctx context.Context) (<-chan provider.StreamEvent, error) {
	...
	content := "<context>\nThis is the previously approved plan. Execute it strictly, using tools to inspect, modify, and verify as needed.\n\n" + s.LastPlan + "\n</context>\n\nExecute the plan now."
	messages := append([]provider.ChatMessage(nil), s.History...)
	messages = append(messages, provider.ChatMessage{Role: provider.RoleUser, Content: content})
	req := agent.RunRequest{Messages: messages, Mode: agent.RunModeExecute, Tools: agent.ToolSet{Mode: agent.ToolSetAll}}
	return s.run(ctx, req, "")
