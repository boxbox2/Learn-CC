...
func TestReviewCommandSubmitsFixedPrompt(t *testing.T) {
	m := newTestModel()
	runner := m.Runner.(*fakeRunner)
	runner.events = closedEvents()
	...
	updated, _ := m.Update(tea.KeyMsg{Type: tea.KeyEnter})
	m = updated.(Model)
	if runner.submitCalled != 1 || !strings.Contains(runner.inputs[0], "review") {
		t.Fatalf("review submit calls=%d inputs=%+v", runner.submitCalled, runner.inputs)
	}
}
