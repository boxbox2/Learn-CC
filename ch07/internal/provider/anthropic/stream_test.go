...
func TestSystemBlocks(t *testing.T) {
	blocks := systemBlocks(provider.ChatRequest{
		SystemPrompt: "stable",
		SystemNotes:  []provider.ChatMessage{{Role: provider.RoleSystem, Content: "note"}},
		Messages:     []provider.ChatMessage{{Role: provider.RoleUser, Content: "user"}},
	})
	if len(blocks) != 2 {
		t.Fatalf("blocks = %+v", blocks)
	}
	if blocks[0].Text != "stable" || blocks[1].Text != "note" {
		t.Fatalf("blocks = %+v", blocks)
	}
}
