...
const reviewPrompt = "Please review the current code changes. Prioritize bugs, behavioral regressions, race conditions, missing tests, and risks. Lead with findings ordered by severity and include concrete file or behavior references when available."
...
func handleReview(ctx context.Context, inv command.Invocation, c command.Controller) (command.Result, error) {
	if err := c.SendUserMessage(ctx, reviewPrompt); err != nil {
		return command.Result{}, err
	}
	return command.Result{Message: reviewPrompt, SentToAI: true}, nil
}
